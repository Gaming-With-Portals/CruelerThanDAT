...
namespace CruelerThanDAT.Systems
{
    ...
    public class CONVERTED_DDS_OBJECT
    {
        public string file_type;
        public int width;
        public int height;
        public byte[] con_data;
        ...
        public static CONVERTED_DDS_OBJECT ConvertDDS(byte[] DDSFile)
        {

            Stream stream = new MemoryStream(DDSFile);
            BinaryReader reader = new BinaryReader(stream);
            // Get Width Info
            reader.BaseStream.Seek(12, SeekOrigin.Begin);
            uint dds_width = reader.ReadUInt32();
            uint dds_height = reader.ReadUInt32();

            reader.BaseStream.Seek(84, SeekOrigin.Begin);
            uint DXT_VERSION = reader.ReadUInt32();
            
            if (DXT_VERSION == 894720068) // DXT5 Header
            {
                CONVERTED_DDS_OBJECT dds_obj = new CONVERTED_DDS_OBJECT();
                dds_obj.con_data = DecompressDxt5(DDSFile, (int)dds_width, (int)dds_height);
                dds_obj.org_data = DDSFile;
                dds_obj.width = (int)dds_width;
                dds_obj.height = (int)dds_height;
                dds_obj.file_type = "DXT5";

                return dds_obj;
            }
            else
            {
                    return new CONVERTED_DDS_OBJECT();
            }
